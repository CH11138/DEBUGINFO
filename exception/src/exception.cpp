    ...
    int buflen, int* offset,bool demangle)
{
    ...
    if (SymbolEngine::decode(addr, buf, buflen, offset, demangle))  //decode
    {
        return true;
    }
    if (offset != NULL)
...
//BOOL GetModuleListTH32(HANDLE hProcess, DWORD dwProcessId);
...
//        if (WindowsDbgHelp::GetModuleInfo(GetCurrentProcess(), stk.AddrPC.Offset, &Module) != FALSE) { // got module info OK
...
//                break;
//            }
//            cout << "************" << "SymType:" << symTypeString << "\tModuleName:" << Module.ModuleName << "*************" << endl;
//        }
    ...
    ULONGLONG fileVersion)
{
    ...
    buffer[1023] = 0; // be sure it is NULL terminated
...
DWORD LoadModule(HANDLE hProcess, LPCSTR img, LPCSTR mod, DWORD64 baseAddr, DWORD size)
{
    ...
    if (dwSize > 0)
    {
        ...
        if (WindowsDbgHelp::GetModuleInfo(hProcess, baseAddr, &Module) != FALSE)
        {
            switch (Module.SymType)
            {
            case SymNone:
...
void create_water1() {
    std::shared_ptr<watert> ptr = nullptr;   //异常
    //ptr->set_ph(100);
    std::cout << ptr->get_ph();
}
