...
public:
    ...
    bool append_directory(const char* directory) {
        ...
        strcpy_s(p,strlen(directory)+1, directory);
        return true;
    }
...
const size_t K = 1024;
typedef SimpleBufferWithFallback<char, MAX_PATH, 8 * K> FileNameBuffer;   //-----MARK
...
static bool decode_locked(const void* addr, char* buf, int buflen, int* offset, bool do_demangle) {
    ...
    if (success = WindowsDbgHelp::symGetSymFromAddr64(::GetCurrentProcess(), (DWORD64)addr, &displacement, pSymbol)) {
        //success = true;
    ...
    if (success == false) {
        ...
        std::cout << "symGetSymFromAddr64 failed,error code: " << error_code << std::endl;
    }
...
static state2  g_state = state2::state_uninitialized;

static void initialize() {

    assert(g_state == state2::state_uninitialized, "wrong sequence");
    g_state = state2::state_error;
...
//资源锁
static CRITICAL_SECTION g_cs;  //这里的struct没有初始化   添加初始化
...
namespace { // Do not export.
    ...
    public:
        SymbolEngineEntry() {
            ::EnterCriticalSection(&g_cs);    ///报错
            if (g_state == state2::state_uninitialized) {
                initialize();
            }
        }
        ~SymbolEngineEntry() {
...
// Called at DLL_PROCESS_ATTACH.  使用该函数初始化CRITICAL_SECTION类型
